                ...
                if (e.DataEntitys != null && e.DataEntitys.Count<DynamicObject>() > 0)
                {
                    foreach (DynamicObject item in e.DataEntitys)
                    {
                        ...
                        if (FBILLTYPEID == "5d18aa0e58407c")
                        {
                            string upsql = "";
                            string UpdatesqlH = "";
                            sql = string.Format(@"select ar.FMODIFYDATE as 修改时间, arf.FALLAMOUNT as 表头价税合计,arel.FBASICUNITQTY,
                                                   arf.FNOTAXAMOUNT as 表头不含税金额 , are.FALLAMOUNTFOR as 表体价税合计,
                                    ...
                                    if (dt2.Rows.Count > 0)
                                    {
                                        for (int j = 0; j < dt2.Rows.Count; j++)
                                        {
                                            ...
                                            if (FSTABLENAME == "T_SAL_OUTSTOCKENTRY")
                                            {
                                                ...
                                                //销售出库单表体
                                                upsql += string.Format(@"/*dialect*/ update T_SAL_OUTSTOCKENTRY set FARFNOTAXAMOUNTFOR=FARFNOTAXAMOUNTFOR-{1},FARFALLAMOUNTFOR=FARFALLAMOUNTFOR-{2} ,FARFQty=FARFQty-{3}  
